...
- 为 agent 暴露包含 32 个公共 Lovart 工具的 MCP server。
...
`signer_stale`、`schema_invalid`、`credit_risk`、`task_failed`、
`content_policy_rejected`、`timeout`、`network_unavailable` 和
`internal_error`。`content_policy_rejected` 只表示远端 task 响应中有审核或
策略拒绝证据；CLI 不会在提交前本地预测或改写 prompt。
...
lovart project artifacts [project_id]
lovart project artifact <artifact_id> --project-id <project_id>
...
lovart task wait <task_id>
lovart download task <task_id>
lovart task canvas <task_id>
```

生成需要已认证会话、已同步的 signer 和 metadata，以及已选择的项目上下文。
`generate` 默认只提交任务并返回 `task_id`；下载和画布写回必须显式执行。

产物查询和下载：

```sh
lovart project artifacts [project_id]
lovart project artifacts [project_id] --task-id <task_id> --detail full
lovart project artifact <artifact_id> --project-id <project_id> --include-raw
lovart task status <task_id>
lovart task list --active
lovart task cancel <task_id> [<task_id>...] --yes
lovart task status <task_id> --detail full
lovart task wait <task_id>
lovart task canvas <task_id> --project-id <project_id>
lovart download task <task_id>
lovart download task <task_id> --index 2 --download-dir ./images
lovart download canvas [project_id] --artifact-id <artifact_id>
lovart download canvas [project_id] --index 3
lovart download canvas [project_id] --task-id <task_id>
lovart download canvas [project_id] --all
```

`download task` 默认下载该 task 的全部图片；`download canvas` 必须显式选择
`--artifact-id`、`--index`、`--task-id` 或 `--all` 之一。canvas 下载只读取远端
project 并写入本地文件，不会修改画布。`task canvas` 只接受已完成 task；主流程用
`task wait` 等待完成，`task status` 保留给脚本轮询和诊断。`task status` 和
`task wait` 查询失败的远端任务时仍然返回 `ok:true`，业务失败会放在 `data.failure`
中；需要产物或写回画布的命令如果遇到审核拒绝会返回 `ok:false` 和
`content_policy_rejected`。如果账号已达到并发上限，先用 `task list --active`
查看占用任务；只有明确要停止任务时才用 `task cancel ... --yes`。
...
lovart jobs finalize ~/.lovart/runs/jobs-<hash> --download --canvas
```

`jobs run` 和 `jobs resume` 会串行提交任务，默认每次提交之间等待 2 秒，避免触发
远端 burst limit。它们还会在提交前检查账号当前运行任务数，默认达到 10 个 active
tasks 时停止本轮提交，剩余请求保留为 pending/quoted；等任务完成后继续运行
`lovart jobs resume ~/.lovart/runs/jobs-<hash>` 即可从未提交处恢复。遇到 429、
rate limit、timeout 或并发上限错误时也会用同样方式 defer。需要主动限制单轮提交
数量时使用 `--submit-limit N`，需要调整节奏时使用 `--submit-interval-seconds N`；
需要调整 active task 上限时使用 `--max-active-tasks N`，`0` 表示关闭这个门禁。
...
- batch submit interval 默认是 2 秒；`--submit-limit 0` 表示不主动限制单轮数量。
- batch active task gate 默认是 10；`--max-active-tasks 0` 表示不检查当前运行任务数。
...
MCP server 声明 protocol version `2024-11-05`，并提供 32 个 tools：

```text
lovart_auth_status
lovart_auth_login
lovart_extension_status
lovart_extension_install
lovart_extension_open
lovart_setup
lovart_models
lovart_config
lovart_balance
...
lovart_canvas_artifacts
lovart_canvas_artifact
lovart_canvas_download
lovart_quote
lovart_generate
lovart_jobs_run
lovart_jobs_status
lovart_jobs_resume
lovart_jobs_finalize
```

`lovart_generate` 在 MCP 中默认只提交任务并返回 `task_id`，不会等待长时间出图。
单任务主流程使用 `lovart_task_wait` 等到终态；`lovart_task_status` 用于脚本轮询和
诊断。批量任务使用 `lovart_jobs_status(refresh=true)` 刷新结果。任务完成后，可用
`lovart_task_download` 下载产物，或用 `lovart_task_canvas` 将产物写回项目画布。
批量任务默认也是异步提交；完成后用 `lovart_jobs_finalize(download=true/canvas=true)`
执行下载和画布写回。`lovart_jobs_run` 和 `lovart_jobs_resume` 同样默认按 2 秒间隔
串行提交，并支持 `submit_interval_seconds`、`submit_limit` 与 `max_active_tasks`
控制提交节奏和 active task 门禁。达到并发上限时，先用 `lovart_task_list` 查看当前
占用任务；只有用户明确要求停止任务时再调用 `lovart_task_cancel`。
如果远端任务因审核/策略拒绝失败，状态响应中的 `failure.code` 会是
