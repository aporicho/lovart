...
class McpConfigTest(unittest.TestCase):
    def test_mcp_status_lists_supported_clients(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            result = mcp_status(clients="all", lovart_path=Path("/tmp/lovart"), home=Path(temp))
        names = {item["client"] for item in result["clients"]}
        self.assertEqual(names, {"codex", "claude", "opencode", "openclaw"})

    def test_codex_dry_run_outputs_toml_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            home = Path(temp)
            result = mcp_install(clients="codex", lovart_path=Path("/tmp/lovart"), home=home, dry_run=True, yes=True)
    ...
    def test_opencode_dry_run_outputs_json_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            home = Path(temp)
            result = mcp_install(clients="opencode", lovart_path=Path("/tmp/lovart"), home=home, dry_run=True, yes=True)
    ...
    def test_codex_unmanaged_config_conflict_requires_force(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            with self.assertRaises(LovartError) as raised:
                mcp_install(clients="codex", lovart_path=Path("/tmp/lovart"), home=home, yes=True)
    ...
    def test_codex_write_creates_backup(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = mcp_install(clients="codex", lovart_path=Path("/tmp/lovart"), home=home, yes=True)
            text = path.read_text()
            backup = Path(result["results"][0]["backup"])
            self.assertIn(str(Path("/tmp/lovart").resolve()), text)
            self.assertTrue(backup.exists())

    def test_clients_none_makes_no_changes(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            home = Path(temp)
            result = mcp_install(clients="none", lovart_path=Path("/tmp/lovart"), home=home, yes=True)
        self.assertEqual(result["mcp_clients_selected"], [])
    ...
    def test_opencode_invalid_json_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            path.write_text("{")
            with self.assertRaises(LovartError) as raised:
                mcp_install(clients="opencode", lovart_path=Path("/tmp/lovart"), home=home, yes=True)
